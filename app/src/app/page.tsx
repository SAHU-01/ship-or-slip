...
import { useMarkets, Market } from "@/context/MarketContext";
...
function MarketCard({ market }: { market: Market }) {
  ...
  return (
    <div className="glass-card group" style={{ padding: "16px", display: "flex", flexDirection: "column", height: "100%", minHeight: "200px" }}>
      ...
      <h3 className="font-display" style={{ fontSize: "14px", fontWeight: 600, color: "white", marginBottom: "12px", lineHeight: "1.3", minHeight: "36px" }}>{market.prTitle}</h3>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", paddingTop: "8px", borderTop: "1px solid rgba(0,245,212,0.2)", marginTop: "auto" }}><span style={{ color: "#9ca3af" }}><span style={{ color: "white", fontWeight: 600 }}>{totalPool.toFixed(2)}</span> SOL</span><span style={{ color: "#6b7280" }}>👥 {market.bettorCount}</span></div>
    </div>
  );
}
...
function CreateMarketModal({ isOpen, onClose, onSuccess }: { isOpen: boolean; onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async () => {
    if (!program || !repo || !prNumber) return; setLoading(true); setError("");
    try { const deadline = Math.floor(Date.now() / 1000) + parseInt(hours) * 3600; await createMarket(program, repo, parseInt(prNumber), deadline); onSuccess(); onClose(); setRepo(""); setPrNumber(""); } catch (err: any) { setError(err.message || "Failed"); } finally { setLoading(false); }
  };
...
export default function HomePage() {
  const router = useRouter(); const { connected } = useWallet(); const { markets, loading, refreshMarkets } = useMarkets(); const [showCreateModal, setShowCreateModal] = useState(false);
  const totalBets = markets.reduce((sum, m) => sum + m.bettorCount, 0); const totalPooled = markets.reduce((sum, m) => sum + m.shipPool + m.slipPool, 0);
  return (
    <div className="min-h-screen circuit-bg relative overflow-hidden">
      ...
      <div className="relative z-10" style={{ maxWidth: "1200px", margin: "0 auto", padding: "24px 24px 120px 24px" }}>
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "48px" }}><div style={{ display: "flex", alignItems: "center", gap: "8px" }}><span style={{ fontSize: "24px" }}>🚀</span><span className="font-display" style={{ fontSize: "20px", fontWeight: 700 }}>Ship or Slip</span></div><WalletMultiButton /></header>
        ...
        <section style={{ marginBottom: "56px" }}>
          ...
          {loading ? <div className="glass-card" style={{ padding: "48px", textAlign: "center" }}><p style={{ color: "#6b7280" }}>Loading...</p></div> : markets.length === 0 ? <div className="glass-card" style={{ padding: "48px", textAlign: "center" }}><div style={{ fontSize: "48px", marginBottom: "16px" }}>🏜️</div><p style={{ color: "#6b7280" }}>No markets yet</p>{connected && <button onClick={() => setShowCreateModal(true)} className="btn-primary" style={{ marginTop: "16px" }}>Create Market</button>}</div> : <div className="markets-grid">{markets.map((m) => <Link key={m.id} href={`/market/${m.id}`} style={{ display: "block" }}><MarketCard market={m} /></Link>)}</div>}
        </section>
      ...
      <CreateMarketModal isOpen={showCreateModal} onClose={() => setShowCreateModal(false)} onSuccess={refreshMarkets} />
