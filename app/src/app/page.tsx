...
interface MarketWithPR extends MarketData {
  prStatus?: PRStatus | null;
}

function MarketCard({ market }: { market: MarketWithPR }) {
  ...
  const isDecided = market.prStatus?.state === "merged" || market.prStatus?.state === "closed";
  ...
  return (
    <div className="glass-card group" style={{ padding: "16px", display: "flex", flexDirection: "column", height: "100%", minHeight: "220px" }}>
      ...
      {isResolved ? (
        <div style={{ 
          padding: "10px 12px", borderRadius: "8px", marginBottom: "12px", textAlign: "center",
          background: market.outcome === "ship" ? "rgba(34,197,94,0.15)" : "rgba(239,68,68,0.15)",
          border: `1px solid ${market.outcome === "ship" ? "rgba(34,197,94,0.4)" : "rgba(239,68,68,0.4)"}`,
        }}>
          <span style={{ fontSize: "14px", fontWeight: 700, color: market.outcome === "ship" ? "#4ade80" : "#f87171" }}>
            {market.outcome === "ship" ? "🚀 SHIPPED!" : "🕳️ SLIPPED!"}
          </span>
        </div>
      ) : isDecided ? (
        <div style={{ 
          padding: "10px 12px", borderRadius: "8px", marginBottom: "12px", textAlign: "center",
          background: market.prStatus?.state === "merged" ? "rgba(34,211,238,0.15)" : "rgba(239,68,68,0.15)",
          border: `1px solid ${market.prStatus?.state === "merged" ? "rgba(34,211,238,0.4)" : "rgba(239,68,68,0.4)"}`,
        }}>
          <span style={{ fontSize: "12px", fontWeight: 700, color: market.prStatus?.state === "merged" ? "#22d3ee" : "#f87171" }}>
            {market.prStatus?.state === "merged" ? "🚀 MERGED" : "🕳️ CLOSED"} - Betting Locked
          </span>
        </div>
      ) : null}
      ...
      <h3 className="font-display" style={{ fontSize: "14px", fontWeight: 600, color: "white", marginBottom: "12px", lineHeight: "1.3", minHeight: "36px" }}>
        {market.prStatus?.title || `PR #${market.prNumber} - ${market.repo.split('/')[1] || market.repo}`}
      </h3>
...
function CreateMarketModal({ isOpen, onClose, onSuccess }: { isOpen: boolean; onClose: () => void; onSuccess: () => void }) {
  ...
  useEffect(() => {
    if (repo && prNumber) {
      ...
      const timer = setTimeout(async () => {
        const status = await fetchPRStatus(repo, parseInt(prNumber));
        setPrStatus(status);
        setChecking(false);
      }, 500);
  ...
  const handleSubmit = async () => {
    ...
    if (prStatus?.state !== "open") {
      setError("Cannot create market for closed/merged PR");
      return;
    }
    ...
    try {
      ...
      alert(`Market created! TX: ${sig.slice(0,20)}...`);
      onSuccess();
      onClose();
      setRepo(""); setPrNumber(""); setPrStatus(null);
    } catch (err: any) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", background: "rgba(0,0,0,0.8)" }}>
      <div className="glass-card" style={{ padding: "24px", width: "100%", maxWidth: "400px", margin: "16px" }}>
        ...
        {checking && <p style={{ fontSize: "12px", color: "#6b7280", marginBottom: "12px" }}>Checking PR...</p>}
        {prStatus && (
          <div style={{ padding: "12px", borderRadius: "8px", background: "rgba(255,255,255,0.03)", marginBottom: "16px" }}>
            <div style={{ fontSize: "13px", fontWeight: 600, color: "white", marginBottom: "4px" }}>{prStatus.title}</div>
            <div style={{ fontSize: "11px", color: "#6b7280" }}>by {prStatus.author}</div>
            <div style={{ marginTop: "8px", fontSize: "11px", fontWeight: 600, color: prStatus.state === "open" ? "#4ade80" : "#f87171" }}>
              {prStatus.state === "open" ? "✅ Open - Ready to create" : "⚠️ Already " + prStatus.state + " - Cannot bet"}
            </div>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: "12px" }}><button onClick={onClose} style={{ flex: 1, padding: "12px", borderRadius: "8px", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", color: "white", cursor: "pointer" }}>Cancel</button><button onClick={handleSubmit} disabled={loading || !repo || !prNumber || prStatus?.state !== "open"} className="btn-primary" style={{ flex: 1, padding: "12px", opacity: prStatus?.state !== "open" ? 0.5 : 1 }}>{loading ? "Creating..." : "Create"}</button></div>
      </div>
    </div>
  );
}
...
export default function HomePage() {
  ...
  const [markets, setMarkets] = useState<MarketWithPR[]>([]);
  ...
  const loadMarkets = useCallback(async () => {
    ...
    // Fetch PR status for each market
    const marketsWithPR = await Promise.all(
      data.map(async (market) => {
        const prStatus = await fetchPRStatus(market.repo, market.prNumber);
        return { ...market, prStatus };
      })
    );
    
    setMarkets(marketsWithPR);
    setLoading(false);
  }, [connection]);
  ...
  const liveMarkets = markets.filter(m => m.status === "open" && m.prStatus?.state === "open");
  const pendingMarkets = markets.filter(m => m.status === "open" && (m.prStatus?.state === "merged" || m.prStatus?.state === "closed"));
      ...
      <div className="relative z-10" style={{ maxWidth: "1200px", margin: "0 auto", padding: "24px 24px 120px 24px" }}>
        ...
        <section style={{ marginBottom: "40px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            <h2 className="font-display" style={{ fontSize: "20px", fontWeight: 700, display: "flex", alignItems: "center", gap: "8px" }}><span style={{ width: "8px", height: "8px", borderRadius: "50%", background: "#4ade80", animation: "pulse 2s infinite" }}></span>Live Markets</h2>
            {connected && <button onClick={() => setShowCreateModal(true)} className="btn-primary" style={{ padding: "8px 16px", fontSize: "12px" }}>+ Create Market</button>}
          </div>
          {loading ? (
            <div className="glass-card" style={{ padding: "48px", textAlign: "center" }}><p style={{ color: "#6b7280" }}>Loading markets...</p></div>
          ) : liveMarkets.length === 0 ? (
            <div className="glass-card" style={{ padding: "48px", textAlign: "center" }}>
              <div style={{ fontSize: "48px", marginBottom: "16px" }}>🎯</div>
              <p style={{ color: "#6b7280", marginBottom: "16px" }}>No live markets</p>
              {connected && <button onClick={() => setShowCreateModal(true)} className="btn-primary">Create First Market</button>}
            </div>
          ) : (
            <div className="markets-grid">{liveMarkets.map((m) => <Link key={m.pubkey} href={`/market/${m.pubkey}?repo=${encodeURIComponent(m.repo)}&pr=${m.prNumber}`} style={{ display: "block" }}><MarketCard market={m} /></Link>)}</div>
          )}
        </section>
        ...
        {pendingMarkets.length > 0 && (
          <section style={{ marginBottom: "40px" }}>
            <h2 className="font-display" style={{ fontSize: "20px", fontWeight: 700, display: "flex", alignItems: "center", gap: "8px", marginBottom: "20px" }}><span style={{ fontSize: "16px" }}>⏳</span>Awaiting Resolution</h2>
            <div className="markets-grid">{pendingMarkets.map((m) => <Link key={m.pubkey} href={`/market/${m.pubkey}?repo=${encodeURIComponent(m.repo)}&pr=${m.prNumber}`} style={{ display: "block" }}><MarketCard market={m} /></Link>)}</div>
          </section>
        )}
        ...
        {resolvedMarkets.length > 0 && (
          <section style={{ marginBottom: "40px" }}>
            <h2 className="font-display" style={{ fontSize: "20px", fontWeight: 700, display: "flex", alignItems: "center", gap: "8px", marginBottom: "20px" }}><span style={{ fontSize: "16px" }}>✅</span>Resolved</h2>
            <div className="markets-grid">{resolvedMarkets.map((m) => <Link key={m.pubkey} href={`/market/${m.pubkey}?repo=${encodeURIComponent(m.repo)}&pr=${m.prNumber}`} style={{ display: "block" }}><MarketCard market={m} /></Link>)}</div>
          </section>
        )}

        <section className="glass-card-purple" style={{ padding: "32px" }}><h2 className="font-display" style={{ fontSize: "20px", fontWeight: 700, textAlign: "center", marginBottom: "24px" }}>How It Works</h2><div className="how-it-works-grid">{[{ emoji: "1️⃣", title: "Pick a PR", desc: "Browse open markets" },{ emoji: "2️⃣", title: "Make Your Call", desc: "🚀 SHIP or 🕳️ SLIP" },{ emoji: "3️⃣", title: "Stake SOL", desc: "On-chain bets" },{ emoji: "4️⃣", title: "Collect", desc: "Winners split pool" }].map((s, i) => <div key={i} style={{ textAlign: "center" }}><span style={{ fontSize: "28px", display: "block", marginBottom: "8px" }}>{s.emoji}</span><h3 className="font-display" style={{ fontWeight: 600, fontSize: "14px", marginBottom: "4px" }}>{s.title}</h3><p style={{ fontSize: "11px", color: "#6b7280" }}>{s.desc}</p></div>)}</div></section>
      </div>
